...
class YoloDetectionNode(Node):
    def __init__(self):
        ...
        package_share_dir = get_package_share_directory("yolo_example_pkg")
        ...
        if using_yolo_det_model:
            detection_model_path = os.path.join(
                package_share_dir, "models", "detection.pt"
            )
        ...
        if using_yolo_seg_model:
            segmentation_model_path = os.path.join(
                package_share_dir, "models", "segmentation.pt"
            )
        ...
        # 發佈 detection 與 segmentation 的處理後影像 Topic
        self.det_image_pub = None
        self.seg_image_pub = None
    ...
    def image_callback(self, msg):
        ...
        if using_yolo_det_model and self.det_model is not None:
            try:
        ...
        if using_yolo_seg_model and self.seg_model is not None:
            try:
    ...
    def get_class_color(self, class_id):
        ...
        return tuple(int(c) for c in rng.randint(0, 256, 3))
    ...
    def draw_bounding_boxes(self, image, results):
        ...
        det_image, points = self.draw_cross(det_image)
    ...
    def draw_masks(self, image, results):
        ...
        mask_image, _ = self.draw_cross(mask_image)
        ...
        for result in results:
            ...
            for box, mask, class_id, conf in zip(boxes, masks, classes, confidences):
                ...
                if np.any(mask_binary):
                    mask_colored = np.zeros_like(mask_image, dtype=np.uint8)
                    mask_colored[mask_binary] = color
                    mask_image = cv2.addWeighted(mask_image, 1.0, mask_colored, 0.5, 0.0)
    ...
    def publish_det_image(self, image, header):
        """將 Detection 影像轉換並發佈到 ROS"""
        if self.det_image_pub is None:
            return
        try:
    ...
    def publish_seg_image(self, image, header):
        """將 Segmentation 影像轉換並發佈到 ROS"""
        if self.seg_image_pub is None:
            return
        try:
            compressed_msg = self.bridge.cv2_to_compressed_imgmsg(image)
            compressed_msg.header = header
